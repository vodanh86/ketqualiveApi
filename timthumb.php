...
if(! defined('FILE_CACHE_SUFFIX') ) 		define ('FILE_CACHE_SUFFIX', '.mava');			// What to put at the end of all files in the cache directory so we can identify them
if(! defined('FILE_CACHE_PREFIX') ) 		define ('FILE_CACHE_PREFIX', 'mava');				// What to put at the beg of all files in the cache directory so we can identify them
if(! defined('FILE_CACHE_DIRECTORY') ) 		define ('FILE_CACHE_DIRECTORY', './cache');				// Directory where images are cached. Left blank it will use the system temporary directory (which is better for security)
if(! defined('TIMTHUMB_MAX_FILE_SIZE') )				define ('TIMTHUMB_MAX_FILE_SIZE', 20485760);						// 10 Megs is 10485760. This is the max internal or external file size that we'll process.  
...
if(! defined('LOCAL_FILE_BASE_DIRECTORY') )	define ('LOCAL_FILE_BASE_DIRECTORY', BASEDIR);			// Thư mục gốc
...
class timthumb {
	...
	public function __construct(){
		...
		$cacheKey = $this->param('src') .'_'. $this->param('w') .'_'. $this->param('h') .'_'. $this->param('zc');
		if($this->isURL){
			$arr = explode('&', $_SERVER ['QUERY_STRING']);
			asort($arr);
			$this->cachefile = $this->cacheDirectory . '/' . FILE_CACHE_PREFIX . $cachePrefix . md5($this->salt . $cacheKey . implode('&', $arr) . $this->fileCacheVersion) . FILE_CACHE_SUFFIX;
		} else {
			...
			$this->cachefile = $this->cacheDirectory . '/' . FILE_CACHE_PREFIX . $cachePrefix . md5($this->salt . $this->localImageMTime . $cacheKey . $this->fileCacheVersion) . FILE_CACHE_SUFFIX;
		}
	...
	protected function serveInternalImage(){
		...
		if($fileSize > TIMTHUMB_MAX_FILE_SIZE){
	...
	public static function curlWrite($h, $d){
		...
		if(self::$curlDataWritten > TIMTHUMB_MAX_FILE_SIZE){
			return 0;
		} else {
			return strlen($d);
		}
