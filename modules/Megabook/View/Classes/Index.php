<?php
    $banners = get_banners('top_book_by_classes');
...
<div class="rm-box">
    <div class="container">
        <h2 class="rm-box-title">
            <span><a href="<?php echo Mava_Url::getPageLink('lop-hoc/'. Mava_String::unsignString($class['title'], '-')); ?>"><?php echo $class['title']; ?></a></span>
            <?php
            $count = 0;
            $item_per_row = 4;
            $count_product = count($class['books']);
        ...
        <div class="rm-product-list-wrap">
            <div class="rm-product-list rm-product-list-tab">
                <div class="rm-product-list-inner clearfix active">
                    <?php
                    if(isset($class['books']) && is_array($class['books']) && count($class['books']) > 0){
                        foreach($class['books'] as $item){
