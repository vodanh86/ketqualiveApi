 ...
 * @Time: 4:42 PM
 */
class Admin_Controller_Subscribes extends Mava_AdminController {
    ...
    public function indexAction(){
        ...
        $skip = ($page-1)*$limit;
        $subModel = $this->_getSubscribeModel();
        $subscribes = $subModel->getList($skip, $limit, $status);
        $baseUrl = Mava_Url::removeParam(Mava_Url::getCurrentAddress(),array('page'));
        $pagination = Mava_View::buildPagination($baseUrl,ceil($subscribes['total']/$limit),$page);
        $viewParams = array(
        ...
        return $this->responseView('Admin_View_Subscribe_List', $viewParams);
    }
    ...
    public function changeStatusAction(){
        ...
        $status = Mava_Url::getParam('status');
        $subModel = $this->_getSubscribeModel();
        if($id > 0 && in_array($status, array('subscribe','unsubscribe')) && $subscribe = $subModel->getById($id)){
            $subDW = $this->_getSubscribeDataWriter();
            $subDW->setExistingData($id);
            $subDW->set('status', $status);
            if($subDW->save()){
                ...
                return $this->responseJson(array(
                    'status' => 1,
                    'subscribe_label' => __('subscribe_status_'. $status),
                    'subscribe_class' => $status_class,
                    'message' => __('subscribe_status_changed')
                ));
            }else{
                return $this->responseJson(array(
                    'status' => -1,
                    'message' => __('can_not_change_status')
                ));
            }
        }else{
            return $this->responseJson(array(
                'status' => -1,
                'message' => __('subscribe_not_found')
            ));
        }
    }
