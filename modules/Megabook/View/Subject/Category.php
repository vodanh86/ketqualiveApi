    ...
    <div class="row">
        <div class="col-md-3">
                    ...
                    <?php
                        if(isset($subjects) && is_array($subjects) && count($subjects) > 0){
                            foreach($subjects as $item){
                                echo '<li'. ($item['id']==$subject['id']?' class="active"':'') .'><a href="'. Mava_Url::getPageLink('mon-hoc/'. Mava_String::unsignString($item['title'], '-')) .'">'. htmlspecialchars($item['title']) .' <i class="fa fa-angle-right"></i></a></li>';
                            }
                        }
                    ?>
                </ul>
            ...
            <div class="rm-product-category-left-banner">
                <?php
                    $banners = get_banners('left_book_by_subject');
