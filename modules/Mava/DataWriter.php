<?php
abstract class Mava_DataWriter
{
    /**
    ...
    {
        $this->_db = (isset($inject['db']) ? $inject['db'] : Mava_Application::getDb());
    ...
    public function setOption($name, $value)
    {
        ...
        {
            throw new Mava_Exception('Cannot set unknown option');
        }
    ...
    public function getOption($name)
    {
        if (!array_key_exists($name, $this->_options))
        {
            throw new Mava_Exception('Cannot get unknown option ' . $name);
        }
    ...
    public function setExistingData($data, $trustInputAsPrimary = false)
    {
        ...
        else
        {
            ...
            {
                throw new Mava_Exception('_getExistingData returned an array but did not include data for the primary table');
            }
        }
    ...
    {
        $this->error(__($this->_existingDataErrorPhrase));
    }
    ...
    public function set($field, $value, $tableName = '', array $options = null)
    {
        ...
        {
            throw new Mava_Exception('Set cannot be called after preSave has been called.');
        }
    ...
    protected function _castValueToType($fieldType, $value, $fieldName, array $fieldData)
    {
        switch ($fieldType)
        {
            ...
            case self::TYPE_SERIALIZED:
                ...
                {
                    throw new Mava_Exception('Value is not unserializable');
                }
            ...
            case self::TYPE_JSON:
                ...
                {
                    throw new Mava_Exception('Value cannot be JSON decoded');
                }
    ...
    protected function _applyFieldValueLimits($fieldType, &$value, array $extraLimits = array())
    {
        ...
        switch ($fieldType)
        {
            ...
            case self::TYPE_BINARY:
                ...
                if (isset($extraLimits['maxLength']) && $strlenFunc($value) > $extraLimits['maxLength'])
                {
                    ...
                    {
                        return __('please_enter_value_using_x_characters_or_fewer', array('count' => $extraLimits['maxLength']));
                    }
                }
                break;
            ...
            case self::TYPE_UINT:
                if ($value < 0)
                {
                    ...
                    {
                        return __('please_enter_positive_whole_number');
                    }
                }
        ...
        switch ($fieldType)
        {
            ...
            case self::TYPE_FLOAT:
                if (isset($extraLimits['min']) && $value < $extraLimits['min'])
                {
                    ...
                    {
                        return __('please_enter_number_that_is_at_least_x', array('min' => $extraLimits['min']));
                    }
                }
                ...
                if (isset($extraLimits['max']) && $value > $extraLimits['max'])
                {
                    ...
                    {
                        return __('please_enter_number_that_is_no_more_than_x', array('max' => $extraLimits['max']));
                    }
                }
                break;
        }
        ...
        {
            return __('please_enter_valid_value');
        }
    ...
    public function error($error, $errorKey = false, $specificError = true)
    {
        ...
        {
            throw new Mava_Exception($error, true);
        }
    }
    ...
    protected function _haveErrorsPreventSave()
    {
        if ($this->_errors)
        {
            ...
            {
                throw new Mava_Exception($this->_errors, true);
            }
        }
    ...
    public function save()
    {
        ...
        if (!$this->_newData)
        {
            ...
            {
                throw new Mava_Exception('Cannot save item when no data has been set');
            }
        }
    ...
    protected function _triggerRequiredFieldError($tableName, $field)
    {
        ...
        {
            $this->error(__($this->_fields[$tableName][$field]['requiredError']), $field, false);
        }
        else
        {

            $this->error(__('please_enter_value_for_required_field_x', array('field' => $field)), $field, false);
        }
    }
    ...
    protected function _delete()
    {
        foreach ($this->_getTableList() AS $tableName)
        {
            ...
            if (!$condition)
            {
                throw new Mava_Exception('Cannot delete data without a condition');
            }
    ...
    public function getModelFromCache($class)
    {
        ...
        {
            self::$_modelCache[$class] = Mava_Model::create($class);
        }
     ...
     *
     * @return Mava_Model_Phrase
     */
    protected function _getPhraseModel()
    {
        return $this->getModelFromCache('Mava_Model_Phrase');
    }

    /**
     * Helper method to get the cache object.
     *
     * @return Mava_Cache|false
     */
     ...
     *
     * @return Mava_DataWriter
     */
    public static function create($class, $errorHandler = self::ERROR_ARRAY, array $inject = null)
    {
        $createClass = Mava_Application::resolveDynamicClass($class, 'datawriter');
        if (!$createClass)
        {
            throw new Mava_Exception("Invalid data writer '$class' specified");
        }
