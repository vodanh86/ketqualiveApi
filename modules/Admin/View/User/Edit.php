<div class="page_width">
    ...
    <form class="mava_form form_edit_user" id="form_edit_user" action="<?php echo Mava_Url::buildLink('admin/users/edit', array('userID' => $user['user_id'])); ?>" method="post" enctype="multipart/form-data">
        <h2 class="mava_form_title"><?php echo __('edit_user'); ?></h2>
        <div class="mava_form_rows">
                    ...
                    <select name="userGroup" id="userGroup">
                        ...
                        if(isset($userGroup) && count($userGroup) > 0){
                            foreach($userGroup as $item){
                                echo '<option value="'. $item['group_id'] .'"'. ($user['user_group_id']==$item['group_id']?" selected":"") .'>'. htmlspecialchars($item['group_title']) .'</option>';
                            }
                        }
                        ?>
                    </select>
            ...
            <dl class="row">
                ...
                <dd><input type="text" class="input_text input_short" name="userBirthday" id="userBirthday" value="<?php echo (isset($user['birthday']) && $user['birthday']>0)?date('d/m/Y',$user['birthday']):""; ?>" placeholder="d/m/Y" /></dd>
            </dl>
            <dl class="row">
                <dt>&nbsp;</dt>
                <dd>
            ...
            <dl class="row mava_form_action">
                ...
                <dd>
                    ...
                    <a href="<?php echo Mava_Url::buildLink('admin/users/detail', array('userID' => $user['user_id'])); ?>" class="mava_button mava_button_gray mava_button_medium"><?php echo __('user_permission'); ?></a>
                </dd>
            </dl>
        </div>
    </form>
    ...
    $(document).ready(function(){
        ...
        $('#form_edit_user').submit(function(){
            if(parseInt($('#userGroup').val()) <= 0){
                ...
                $('#userEmail').focus();
                return false;
            }else if($('#userPassword').val() != "" && $('#userPassword').val() != $('#userRePassword').val()){
