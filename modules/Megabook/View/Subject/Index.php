<?php
    $banners = get_banners('top_book_by_subject');
...
<?php
    ...
    if(isset($subjects) && is_array($subjects) && count($subjects)){
        foreach($subjects as $subject){
?>
<div class="rm-box">
    <div class="container">
        <h2 class="rm-box-title">
            <span><a href="<?php echo Mava_Url::getPageLink('mon-hoc/'. Mava_String::unsignString($subject['title'], '-')); ?>"><?php echo $subject['title']; ?></a></span>
            <?php
            $count = 0;
            $item_per_row = 4;
            $count_product = count($subject['books']);
        ...
        <div class="rm-product-list-wrap">
            <div class="rm-product-list rm-product-list-tab">
                <div class="rm-product-list-inner clearfix active">
                    <?php
                    if(isset($subject['books']) && is_array($subject['books']) && count($subject['books']) > 0){
                        foreach($subject['books'] as $item){
