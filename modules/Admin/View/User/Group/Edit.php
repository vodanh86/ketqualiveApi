<div class="page_width">
    ...
    <form class="mava_form form_edit_user_group" id="form_edit_user_group" action="<?php echo Mava_Url::buildLink('admin/users/group_edit', array('groupID' => $group['group_id'])); ?>" method="post" enctype="multipart/form-data">
        <h2 class="mava_form_title"><?php echo __('edit_group'); ?></h2>
        <div class="mava_form_rows">
            <dl class="row">
                <dt><?php echo __('title'); ?></dt>
                <dd><input type="text" class="input_text input_medium" name="groupTitle" id="groupTitle" value="<?php echo htmlspecialchars($group['group_title']); ?>" /></dd>
            </dl>
            <dl class="row">
                <dt><?php echo __('sort_order'); ?></dt>
                <dd class="mava_spinbox">
                    <input type="text" class="input_text input_short mava_spinbox_value" data-step="1" data-min="0" value="<?php echo (int)$group['group_id']; ?>" name="groupSortOrder" id="groupSortOrder">
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $('#button_submit_edit').click(function(){
            $('#form_edit_user_group').submit();
        });
        $('#form_edit_user_group').submit(function(){
            if($('#groupTitle').val()==''){
                $('#groupTitle').focus();
                return false;
            }else{
