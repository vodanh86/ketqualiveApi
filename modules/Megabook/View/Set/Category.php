    ...
    <div class="row">
        <div class="col-md-3">
            <div class="rm-product-category-menu">
                <ul>
                    <?php
                        if(isset($sets) && is_array($sets) && count($sets) > 0){
                            foreach($sets as $item){
                                echo '<li'. ($item['id']==$set['id']?' class="active"':'') .'><a href="'. Mava_Url::getPageLink('bo-sach/'. Mava_String::unsignString($item['title'], '-')) .'">'. htmlspecialchars($item['title']) .' <i class="fa fa-angle-right"></i></a></li>';
                            }
                        }
                    ?>
                </ul>
            ...
            <div class="rm-product-category-left-banner">
                <?php
                    $banners = get_banners('left_book_by_set');
